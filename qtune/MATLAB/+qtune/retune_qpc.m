...
gate = input_args.gate;
scan = qtune.makePythonChargeLineScan(current_qpc_position, range, gate, 1280, .0005, 1, 'DecaDAC');
scan.figure = 32939;
scan.cleanupfn(2).fn =  @smaconfigwrap;
scan.cleanupfn(2).args = {@smset, {gate}, current_qpc_position};
qpc_line = smrun(scan, [file_name, '_',  ['scan_' gate]]);
...
if abs(x(idx)-sensor_pretuned)> 100e-3
	for i = 1:8
      smset(gatechannel(i),pretuned_point(i))
  end
	smset(gate, sensor_pretuned);
	error('The sensing dot is being tuned more than 100mV away from its original position!')
...
new_qpc_position = struct('qpc', x(idx), 'signal_strength', abs(M));
